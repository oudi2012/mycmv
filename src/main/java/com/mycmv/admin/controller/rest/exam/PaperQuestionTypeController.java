...
public class PaperQuestionTypeController {
    ...
    @ResponseBody
    @GetMapping("pageList")
    public ResponseObject list(AbstractUser user, PaperQuestionType item, int pageIndex, int pageSize) {
    ...
    @ResponseBody
    @PostMapping("create")
    public ResponseObject create(AbstractUser user, @RequestBody PaperQuestionType item) {
        logger.info("用户 {} ，访问 {} , 参数：{}", user.getUserName(), "exam/pageQuestionType/create", JSON.toJSON(item));
        ResponseObject resObj = new ResponseObject();
    ...
    @ResponseBody
    @PostMapping("edit")
    public ResponseObject edit(AbstractUser user, @RequestBody PaperQuestionType item) {
